...
# Deliberately NOT `cloudflared service install`.
#
# That registers the service with a bare command line and no config path, and
# it runs as LocalSystem, which reads config from its own profile rather than
# yours. The result is the worst kind of failure: the service reports Running
# while the tunnel has no connections at all, the hostname times out, and
# nothing in any log explains why. It also wedged in StopPending on this
# machine and had to be uninstalled.
#
# A scheduled task under your own account runs `tunnel run` explicitly against
# the config we just wrote, needs no elevation, and is the same mechanism the
# relay already uses.
Write-Host "Registering the tunnel as a scheduled task..."
